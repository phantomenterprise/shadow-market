...
export default function StreetTrader() {
  ...
  const profit = netWorth - config.startCash;
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1.4fr]">
          <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-5">
                ...
                <button
                  ...
                  className={`rounded-full border px-3 py-1 text-xs transition ${
                    loc.id === location
                      ...
                      : 'border-white/10 bg-slate-950/60 text-slate-200 hover:border-red-400/70'
                  }`}
                >
                  {loc.icon} {loc.name}
                </button>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-slate-950/60 p-3 text-xs text-slate-300">
              <p className="font-semibold text-slate-200">Ledger</p>
              <div className="mt-2 flex items-center justify-between">
                <span>Net Worth</span>
                <span className="font-semibold text-slate-100">{config.currency}{netWorth.toLocaleString()}</span>
              </div>
              <div className="mt-1 flex items-center justify-between">
                <span>Profit / Loss</span>
                <span className={`font-semibold ${profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {profit >= 0 ? '+' : ''}{config.currency}{profit.toLocaleString()}
                </span>
              </div>
            </div>
          </div>
